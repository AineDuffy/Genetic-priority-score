...
Firthreg_weights<-mclapply(c(paste0('CVsample',rep(1:5))), function(CVsample){
  ...
  firth_mod <- try(logistf(as.formula(paste0('mi ~ ',Predictors)), data=OT_dataset, firth = TRUE))##Run firth regression across each CV training dataset to get weights
...
Genescore_sum<-lapply(c(paste0('CVsample',rep(1:5))), function(CVsample){
  ...
  Firth_weights=as.data.frame(t(Firth$beta))x
  OT_dataset_20=fread(paste0('OT_drugdataset_20_CV', CVsample, '.txt'),data.table=F) # OT remaining 20% test set 
  OT_dataset_20_gene_phenotype=OT_dataset_20[, grepl(paste0('\\bgene\\b|parentterm|genescores_|phenotype_'), colnames(OT_dataset_20))] %>% distinct() #extract gene, parentterm and genetic predictor columns
...
max_filetype<-do.call(rbind,lapply(c(paste0('CVsample',seq(1:5))), function(samplecv){   
  ...
  Dataset_genescores=merge(OT_dataset_20[c('drugname','gene','parentterm','category','mi')] ,genescorefile, by=c('gene', 'parentterm') ) # combine genescore sum file with drug,mi data for each test set 
  #run logistic model 
...
Genescore_sum<-lapply(c('Sider','Allgenes'), function(valdataset){
  ...
  genescorefile_drugs=merge(Validation_dataset[c('drugname','gene','parentterm','category','mi')],Genescores_beta_genetic_genept, by=c('gene', 'parentterm'))
...
Simulate_MI_prioritization<-lapply(seq(2,trunc(max(Slidingwindow$OR)),1), function(threshold){
 #step 1. Randomly select 1000 gene-phenotypes with a high GPS and get percentage of sampled gene-phenotype pairs with MI
  ...
  Null_distribution<- mclapply(seq(1,nrow(Randomgenept),1), function(it){
    #go through each gene-phenotype from random selection, match on phenotype and then randomly select gene target from dataset with out using GPS.
